...
public class Main {
	...
	public static void smapify(final String filename, final String relPathPrefix, final String javaFile, String classFileName) {
		...
		try {
			...
			X10DTCorePlugin.getInstance().logException("Exception encountered while rewriting .class file '" + classFileName + "'", e);
		} finally {
			try {
			    if (fw != null) {
			        fw.close();
				...
				X10DTCorePlugin.getInstance().logException("Exception encountered while closing .class file '" + classFileName + "'", e);
			}
		}
	}
